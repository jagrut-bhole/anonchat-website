...
>(({ className, showTooltip = false, tooltipContent, ...props }, ref) => {
  ...
  const renderThumb = (value: number) => {
    const thumb = (
      <SliderPrimitive.Thumb
        className="block h-5 w-5 rounded-full border-2 border-white bg-zinc-950 shadow-md ring-offset-background transition-transform hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 data-[disabled]:cursor-not-allowed cursor-grab active:cursor-grabbing"
  ...
  return (
    <SliderPrimitive.Root
      ...
      {...props}
    >
      <SliderPrimitive.Track className="relative grow overflow-hidden rounded-full bg-zinc-800/80 border border-white/10 data-[orientation=horizontal]:h-2.5 data-[orientation=vertical]:h-full data-[orientation=horizontal]:w-full data-[orientation=vertical]:w-2.5">
